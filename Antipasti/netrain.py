...
def prep(ist=None, soll=None, ignoreclass0=False, isautoencoder=False, clip=False):
    ...
    if not clip:
        ...
        # If clip is a tuple, read in clip bounds
        clipbounds = (0.01, 0.99)
        if ist is not None and soll is not None:
            return T.clip(istfc, *clipbounds), T.clip(sollfc, *clipbounds)
        if ist is not None:
            return T.clip(istfc, *clipbounds)
        if soll is not None:
            return T.clip(sollfc, *clipbounds)
...
def repulsive_regularization(params, scale=1e-2, eps=0.001):
    """
    This function can be used to add a repulsive force between the filters in order to encourage them to learn different
    filters. The scale was set to be 1e-3 in order to scale the value of the repulsive regularization to similar values
    as the l2 regularization.

    1/eps: indicates the largest repulsion term allowed per entire filerbank.

    :type params: list
    :param params: List of parameters

    :type scale: float
    :param scale: Scaling term (to weight the relative importance in combined cost)

    :type eps: float
    :param eps: Fuzz factor
    """

    # Define function to compute the repulsive term
    def red_params(param):
        if param.ndim == 4 or param.ndim == 5:
            flat = T.flatten(param, outdim=2)
        elif param.ndim == 1:
            flat = T.reshape(param, (param.shape[0], 1), ndim=2)
        else:
            raise NotImplementedError("This regularization scheme is only implemented for 1 (biases), 4 and 5-D "
                                      "parameter tensors.")

        # Normalize filters to unit vectors
        norms = T.sqrt(T.sum(flat**2, axis=1).dimshuffle(0, 'x'))
        flat = flat/T.clip(norms, eps, np.inf)

        cor_streng = 0.5 * T.mean((flat.reshape((flat.shape[0], 1, -1)) - flat.reshape((1, flat.shape[0], -1)))**2)
        rep = 1./T.clip(cor_streng, eps, np.inf)
        return rep

    repterm = sum([red_params(param) for param in params])

    return scale * repterm
