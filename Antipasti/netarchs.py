...
class model(object):
    ...
    def __init__(self):
        ...
        # Baggage (for the user to store references to variable)
        self.baggage = {}
    ...
    @property
    def isautoencoder(self):
        return self._isautoencoder

    @isautoencoder.setter
    def isautoencoder(self, value):
        if value:
            warn("isautoencoder flag is deprecated.")
    ...
    def error(self, ist=None, soll=None, ignoreclass0=False, modeltype='classification'):
        ...
        # FIXME Doesn't unpack pist and psoll when ist and soll are lists
        # Prepare data
    ...
    def cost(self, ist=None, soll=None, params=None, method='ce', isautoencoder=False, regularizer='lp', vectorize=True,
        ...
        # Determine whether the model should be an autoencoder (False)
    ...
    def compile(self, what='trainer', isautoencoder=None, extrarguments=None, extraoutputs=None,
                compilekwargs=None):
        """
        :type what: str
        ...
        :param extrarguments: Extra arguments to the compiled function. Should be an ordered dict (for your own sanity).
        ...
        :type extraoutputs: dict
        :param extraoutputs: Extra outputs from the compiled function.
                             - Keys must be strings (or names)
                             - Values must be theano variables.

        :type compilekwargs: dict
        :param compilekwargs: Keyword arguments for the theano (Antipasti) compile function.
        """

        if isautoencoder is None:
            isautoencoder = self.isautoencoder

        if extrarguments is None:
            extrarguments = {}

        if compilekwargs is None:
            compilekwargs = {}

        if what in ["trainer", "all"]:
            # Build dict of outputs
            outputs = {'C': self.C, 'L': self.L, 'E': self.E, 'dC': self.dC}
            # Add extra outputs
            if extraoutputs is not None:
                outputs.update(extraoutputs)

            # Compile classifier trainer (take x and y to train for a batch).
            classifiertrainer = A.function(inputs=pyk.obj2list(self.x) + pyk.obj2list(self.yt) +
                                                   extrarguments.keys(),
                                           outputs=outputs,
        ...
        if what in ["inference", "all"]:
            if extraoutputs is None:
                outputs = self.y
            else:
                outputs = extraoutputs
                outputs.update({'y': self.y})

            # Compile classifier (to predict)
            classifier = A.function(inputs=pyk.obj2list(self.x),
                                     outputs=outputs,
                                     allow_input_downcast=True,
        ...
        if what in ["error", "all"]:
            if extraoutputs is None:
                outputs = self.E
            else:
                outputs = extraoutputs
                outputs.update({'vaE': self.E})

            # Compile error (or accuracy function)
            classifiererror = A.function(inputs=pyk.obj2list(self.x) + pyk.obj2list(self.yt),
                                          outputs=outputs,
                                          allow_input_downcast=True,
                                          on_unused_input='warn')
    ...
    def fit(self, trX, trY=None, numepochs=100, maxiter=np.inf, verbosity=0,
            vaX=None, vaY=None, validateevery=None, recompile=False, nanguard=True, extrarguments=None,
            extraoutputs=None, trainingcallbacks=None, validationcallbacks=None, backupparams=True,
            backupbestparams=True, log=None, relay=None):
        """
        :type trX: generator
        ...
        :type verbosity: int
        :param verbosity: Verbosity. 0 for silent execution, 5 for full verbosity.
        ...
        :type trainingprinter: list of callables
        :param trainingprinter: Training Monitors (see netools for a few pre-implemented monitors)

        :type validationprinter: list of callables
        :param validationprinter: Validation Monitors (see netools for a few pre-implemented monitors)
        ...
        # Set trY to -1 if it's None
        trY = -1 if trY is None else trY
        vaY = -1 if vaY is None else vaY
        ...
        if trainingcallbacks is None:
            trainingcallbacks = tk.defaultcallback(verbosity)
        if validationcallbacks is None:
            validationcallbacks = tk.defaultcallback(verbosity)
        ...
        # Compile trainers
        if (self.classifiertrainer is None) or recompile:
            if verbosity >= 1:
                print("Compiling Trainer...")
                
            self.compile(what="trainer", extrarguments=extrarguments, extraoutputs=extraoutputs)

        # Defaults for extrarguments and extraoutputs
        if extrarguments is None:
            extrarguments = {}

        # Set up validation
        validate = not(validateevery is None or validateevery == 0 or vaY is None or vaX is None)
        ...
        if self.classifiererror is None and validate:
            if verbosity >= 1:
                print("Compiling Validation Function...")
            
            self.compile(what="error", isautoencoder=False, extrarguments=extrarguments, extraoutputs=extraoutputs)
        ...
        for epoch in range(numepochs):
            ...
            if log is not None:
                log.write("Training Epoch: {} of {}".format(epoch, numepochs))
            ...
            while True:
                ...
                if numiter >= maxiter:
                    ...
                    if log is not None:
                        log.write("Maximum number of iterations exhausted, aborting...")
                    break
                ...
                try:
                    ...
                    if log is not None:
                        log.write("Generator(s) exhausted...")
                    break
                ...
                try:
                    ...
                    C, L, E = res['C'], res['L'], res['E']
                    dC = res['dC']
                except Exception as e:
                    # Save network parameters
                    self.save(nameflags='--iter-{}-mayday'.format(numiter))
                    # UI
                    if verbosity >= 1:
                        print("Exception raised, backing up parameters as {}...".format(self.lastsavelocation))
                    if log is not None:
                        log.write("Exception raised, backing up parameters as {}...".format(self.lastsavelocation))
                ...
                if nanguard and (np.isnan(C) or np.isnan(L)):
                    ...
                    if log is not None:
                        log.write("NaNs detected, aborting training...")
                    skipepoch = True
                    break

                # Provision to add arbitrary callbacks
                ...
                if validate and numiter % validateevery == 0 and numiter != 0:
                    ...
                    if vaE is not None:
                        if isinstance(vaE, dict):
                            ...
                            iterstat.update({'vaE': vaE})

                        # Call validation callbacks
                        validationcallbacks(**iterstat)

                    validationlog.append(iterstat['vaE'])
                ...
                if backupparams:
                    ...
                    if isinstance(backupparams, int):
                        if numiter % backupparams == 0 and numiter is not 0:
                            ...
                            if log is not None:
                                log.write("Backing up parameters (routine)...")
                            self.save(nameflags='--iter-{}-routine'.format(numiter))
                            if verbosity >= 5:
                                print("Backed up parameters (routine) to {}.".format(self.lastsavelocation))
                            if log is not None:
                                log.write("Backed up parameters (routine) to {}.".format(self.lastsavelocation))
                    ...
                    if validate and None not in validationlog and len(validationlog) > 1:
                        if validationlog[-1] < min(validationlog[:-1]) and numiter % validateevery == 0 and numiter != 0:
                            ...
                            if backupbestparams:
                                ...
                                if log is not None:
                                    log.write("Backing up best set of parameters...")
                                self.save(nameflags='--iter-{}-best'.format(numiter))

                                if verbosity >= 5:
                                    print("Backed up parameters (best) to {}.".format(self.lastsavelocation))
                                if log is not None:
                                    log.write("Backed up parameters (best) to {}.".format(self.lastsavelocation))
    ...
    def save(self, where=None, nameflags=''):
        if where is None:
...
class layertrainyard(model):
    ...
    def __getitem__(self, item):
        if isinstance(item, int):
            # trainyard[item] might be a list. If that's indeed the case, convert to layertrainyard.
            sl = self.trainyard[item]
            if isinstance(sl, list):
                return layertrainyard([sl])
            else:
                return sl
        elif isinstance(item, slice):
