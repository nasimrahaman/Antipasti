...
sys.path.append('/export/home/nrahaman/Python/Repositories/Antipasti/Projects/Albert/Boilerplate')
import prepfunctions_albert
...
def buildpreptrains(**prepconfig):
    ...
    pf = prepfunctions_albert.prepfunctions()
    ...
    if ffd(prepconfig, 'make-wmap', False):
        ...
        ptY = pk.preptrain([pf['time2channel' + ('2D' if prepconfig['is2D'] else '')],
                            pf['seg2membrane'](), pf['disttransform'](**prepconfig['edt-params']), pk.cast('float32')])
...
def load(**loadconfig):
    ...
    # Load raw data
    volX = ndu.fromh5(path=loadconfig['X']['path'], datapath=loadconfig['X']['h5path'],
    ...
    # Load ground truth
    volY = ndu.fromh5(path=loadconfig['Y']['path'], datapath=loadconfig['Y']['h5path'],
...
if __name__ == '__main__':
    ...
    if not isfatchicken:
        ...
        # TODO Set dataconf on fatchicken
        args = argparse.Namespace(dataconf='/mnt/localdata02/nrahaman/Neuro/ConvNet-Backups/Albert/ICv1-0/'
                                           'Configurations/i1/dataconf.yml')
