...
def build(numinp=3, numout=3, wmap=False, binarize=False, parampath=None, convertoldtype=False):
    ...
    if parampath is not None:
        parampath = glob.glob(parampath)[0]
        print("[+] Loading pretrained parameters from {}.".format(parampath))
        interceptorv1.load(parampath)
        # Convert old type parameters to new, if required to
        if convertoldtype:
            print("[-] Converting from old type parameters.")
            # Gather old parameters
            oldW = interceptorv1.params[-2].get_value()
            oldb = interceptorv1.params[-1].get_value()
            # Make new parameters
            newW = oldW[1:, ...]
            newb = oldb[1:]
            # Set parameters
            interceptorv1.trainyard[-1][-1].W.set_value(newW)
            interceptorv1.trainyard[-1][-1].b.set_value(newb)
    else:
